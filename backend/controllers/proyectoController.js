...
const buscarColaborador = async (req, res) => {
  ...
  const usuario = await Usuario.findOne({email}).select('-password -token -__v -confirmado -createdAt -updatedAt')
  if(!usuario){
...
const agregarColaborador = async (req, res) => {
  ...
  if(proyecto.creador.toString() === usuario._id.toString()){
    const error = new Error('El Creado del Proyecto no puede ser colaborado')
    return res.status(404).json({msg:error.message})
  }
  ...
  await proyecto.save()
  res.json({msg: "Colaborado Agregado Correctamente"})
